...
class cosmicsimage:
        ...
        def __str__(self):
                ...
                """
                stringlist = [
                        'Input array : ({:d}, {:d}), {:s}'.format(self.rawarray.shape[0], self.rawarray.shape[1], self.rawarray.dtype.name),
                        'Current cosmic ray mask : {:d} pixels'.format(np.sum(self.mask))
                        ]
                
                if self.pssl != 0.0:
                        stringlist.append('Using a previously subtracted sky level of {:f}'.format(self.pssl))
                        
                #if self.satstars.any != None:
                if type(self.satstars) is np.ndarray:
                        stringlist.append('Saturated star mask : {:d} pixels'.format(np.sum(self.satstars)))
        ...
        def labelmask(self, verbose = None):
                ...
                # We morphologically dilate the mask to generously connect "sparse" cosmics :
        ...
        def findsatstars(self, verbose = None):
                ...
                if verbose:
                                print('We have {:d} saturated stars.'.format(nsat))
        ...
        def lacosmiciteration(self, verbose = None):
                ...
                if verbose:
                        print('{:d} candidate pixels'.format(nbcandidates))
                ...
                if type(self.satstars) is np.ndarray:
                        ...
                        if verbose:
                                print('{:d} candidate pixels not part of saturated stars'.format(nbcandidates))
                ...
                if verbose:
                        print('{:d} remaining candidate pixels'.format(nbcosmics))
                ...
                if verbose:
                        print('{:d} pixels detected as cosmics'.format(nbfinal))
        ...
        def findholes(self, verbose = True):
                ...
                if verbose:
                        print('{:d} hole pixels found'.format(np.sum(holes)))
        ...
        def run(self, maxiter = 4, verbose = False):
                ...
                print('Starting {:d} L.A.Cosmic iterations ...'.format(maxiter))
                for i in range(1, maxiter+1):
                        print('Iteration {:d}'.format(i))
                        
                        iterres = self.lacosmiciteration(verbose=verbose)
                        print('{:d} cosmic pixels ({:d} new)'.format(iterres['niter'], iterres['nnew']))
...
def fromfits(infilename, hdu = 0, verbose = True):
        ...
        """        
        with fits.open(infilename) as hdulist:
                #pixelarray, hdr = pyfits.getdata(infilename, hdu, header=True)
                hdr = hdulist[hdu].header
                pixelarray = hdulist[hdu].data
                pixelarray = np.asarray(pixelarray).transpose()
                pixelarrayshape = pixelarray.shape
                if verbose :
                        print('FITS import shape : ({:d}, {:d})'.format(pixelarrayshape[0], pixelarrayshape[1]))
                        print('FITS file BITPIX : {:d}'.format(hdr['BITPIX']))
...
def tofits(outfilename, pixelarray, hdr = None, verbose = True):
        ...
        pixelarrayshape = pixelarray.shape
        if verbose :
                print('FITS export shape : ({:d}, {:d})'.format(pixelarrayshape[0], pixelarrayshape[1]))
...
def rebin(a, newshape):
        ...
        factor_int = factor.astype(int)
        newshape_int = newshape.astype(int)
        evList = ['a.reshape('] + \
                 ['newshape_int[{:d}],factor_int[{:d}],'.format(i,i) for i in range(lenShape)] + \
                 [')'] + ['.sum({:d})'.format(i+1) for i in range(lenShape)] + \
                 ['/factor_int[{:d}]'.format(i) for i in range(lenShape)]
